...
from zipline.data.fx import FXRateReader
import zipline.pipeline.domain as domain

class JPRateReader(implements(FXRateReader)):
...
def build_pipeline_engine(bundle_data, trading_calendar):
    ...
    engine = SimplePipelineEngine(
        ...
        # cf. https://github.com/quantopian/zipline/issues/2754
...
bundle_data = bundles.load('csvdir')
...
# %%
universe_end_date = pd.Timestamp('2021-03-31', tz='UTC')
...
# %%
print("Original Dimension : {}".format(five_year_returns.shape))
print("Dimension After PCA: {}".format(pca.components_.shape))

# %% [markdown]
# note: We can see the the dimension size changed from 1256 -> 20 as set in the num_factor_exposures.
# Let's see how the data is affected after PCA.
# %% [markdown]
...
# %%
print("Explianed var ratio :", np.sum(pca.explained_variance_ratio_))
...
# %% [markdown]
# note: As below, as the first factor dominates, the explained variance ratios is still above 0.5 (we don't evaluate if this is large or small, though) if the num_factor_exposures values is above or equal to 8.

# %%
num_factor_exposures = 8
pca2 = fit_pca(five_year_returns, num_factor_exposures, 'full')
pca2.components_
print("Explianed var ratio :", np.sum(pca2.explained_variance_ratio_))

# %% [markdown]
# To continue rever the number of factor exposures back to 20.
...
# %% [markdown]
# Seems much efficient than original dimensions.

# %%
five_year_returns.cumsum().plot(legend=None)
...
def predict_portfolio_risk(factor_betas, factor_cov_matrix, idiosyncratic_var_matrix, weights):
    ...
    #print(factor_betas.shape)
    #print(factor_cov_matrix.shape)
    #print(idiosyncratic_var_matrix.shape)
    #print(weights.shape)
    #print(factor_betas.dot(factor_cov_matrix.dot(factor_betas.T)).shape)
    #display(idiosyncratic_var_matrix)
    #display(factor_betas.dot(factor_cov_matrix.dot(factor_betas.T)))
    
    #TODO: Implement function
    ret_cov= factor_betas.dot(factor_cov_matrix.dot(factor_betas.T)).values + idiosyncratic_var_matrix.values
    #print(ret_cov.shape)
    np_weights = weights.values

    #print(ret_cov.shape)
    #print((weights.T.dot(ret_cov).dot(weights)).shape)
    
    return np.sqrt(np_weights.T.dot(ret_cov).dot(np_weights))[0][0]
...
def sharpe_ratio(factor_returns, annualization_factor):
    ...
    return pd.Series(annualization_factor*factor_returns.mean()/factor_returns.std())
...
# 
# **Answer:** Seeing the factor rank autocorrelation (FRA), momentum factor is stable, and so somoothing will not affect significantly. Sharpe ratio of Momentum_1YR is 1.13 and relatively good, and contribute to return for the risk already.
# %% [markdown]
...
class OptimalHoldings(AbstractOptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        #TODO: Implement function
        obj = cvx.Maximize(np.array(alpha_vector.T)*weights)
    ...
    def _get_constraints(self, weights, factor_betas, risk):
        ...
        #TODO: Implement function
        constraints = [risk <= self.risk_cap**2,                     np.array(factor_betas.T) * weights <= self.factor_max,                     np.array(factor_betas.T) * weights >= self.factor_min,                     sum(weights) == 0,                     sum(cvx.abs(weights)) <=1,                     weights <= self.weights_max,                     weights >= self.weights_min]
        return constraints
...
class OptimalHoldingsRegualization(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        #TODO: Implement function
        obj = cvx.Maximize(np.array(alpha_vector.T) * weights - self.lambda_reg * cvx.norm(weights, 2))
...
class OptimalHoldingsStrictFactor(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        x_star = (alpha-alpha.mean())/sum(abs(alpha))
        obj = cvx.Minimize(cvx.norm(weights - x_star.reshape(len(x_star)), 2))
